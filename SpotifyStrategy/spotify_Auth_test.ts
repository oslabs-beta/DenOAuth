...
Deno.test("Spotify's createLink method should return the correct url", () => {
  ...
  assertEquals(
      dummy, 
      `https://accounts.spotify.com/authorize?client_id=688zz8dnnxjo4t&scope=user-read-email&response_type=code&redirect_uri=http://localhost:3000/auth/spotify/callback`
    );
});
...
Deno.test("parsedCode should parse the URL correctly", () => {
  ...
  const fakeStringPathName = (`?code=${randomizedCode}`)
